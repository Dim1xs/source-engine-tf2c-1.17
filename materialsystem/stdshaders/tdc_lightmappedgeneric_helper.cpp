...
#include "convar.h"
#include "tdc_lightmappedgeneric_ps30.inc"
#include "tdc_lightmappedgeneric_vs30.inc"
...
static ConVar r_csm_blend_tweak( "csm_blend_tweak", "15" );
								 ...
								 )
{
	...
	if ( pShaderShadow || ( ! pContextData ) || pContextData->m_bMaterialVarsChanged  || hasFlashlight )
	{
		...
		//{
		//	for (int i = 0; i < 10; ++i)
		//	Warning( "ERROR: bumpmap unsupported!\n" );
		//	Assert( 0 );
		//}
		...
		//{
		//	for (int i = 0; i < 10; ++i)
		//	Warning( "ERROR: outline unsupported!\n" );
		//	bHasOutline = false;
		//	Assert( 0 );
		//}
		...
		//{
		//	for (int i = 0; i < 10; ++i)
		//	Warning( "ERROR: softedges unsupported!\n" );
		//	bHasSoftEdges = false;
		//	Assert( 0 );
		//}
...
//		if( hasLightmap )
				...
				DECLARE_STATIC_VERTEX_SHADER( tdc_lightmappedgeneric_vs30 );
				...
				SET_STATIC_VERTEX_SHADER( tdc_lightmappedgeneric_vs30 );


				//if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
				DECLARE_STATIC_PIXEL_SHADER( tdc_lightmappedgeneric_ps30 );
				...
				SET_STATIC_PIXEL_SHADER( tdc_lightmappedgeneric_ps30 );
	...
	DYNAMIC_STATE
	{
		...
		DECLARE_DYNAMIC_VERTEX_SHADER( tdc_lightmappedgeneric_vs30 );
		...
		SET_DYNAMIC_VERTEX_SHADER_COMBO( CSM, iCascadedShadowCombo );
		SET_DYNAMIC_VERTEX_SHADER_CMD( DynamicCmdsOut, tdc_lightmappedgeneric_vs30 );
		...
		float envmapContrast = params[info.m_nEnvmapContrast]->GetFloatValue();
		//if ( g_pHardwareConfig->SupportsPixelShaders_2_b() )
			DECLARE_DYNAMIC_PIXEL_SHADER( tdc_lightmappedgeneric_ps30 );
			...
			SET_DYNAMIC_PIXEL_SHADER_COMBO( CSM, iCascadedShadowCombo );
			SET_DYNAMIC_PIXEL_SHADER_CMD( DynamicCmdsOut, tdc_lightmappedgeneric_ps30 );
		//else
		//{
