...
void CBasePlayer::ValidateCurrentObserverTarget( void )
{
	if ( !IsValidObserverTarget( m_hObserverTarget.Get() ) )
	{
		...
		{
#if !defined( TF_DLL ) && !defined( TDC_DLL )
...
#ifdef TDC_DLL
			// TODO: This prevents clients from crashing the server by passing bad coordinates.
			// This fix will likely be added in the SDK update so remove when needed.
			if ( !IsEntityPositionReasonable( origin ) || !IsEntityQAngleReasonable( angle ) )
				return true;
#endif
