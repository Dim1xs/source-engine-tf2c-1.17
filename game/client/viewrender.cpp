...
#ifdef TDC_CLIENT_DLL
	// Don't run TDC at all on anything below 9.0
	if ( g_pMaterialSystemHardwareConfig->GetDXSupportLevel() < 90 )
	{
		// We know they were running at least 8.0 when the game started...we check the 
		// value in ClientDLL_Init()...so they must be messing with their DirectX settings.
		if ( ( Q_stricmp( COM_GetModDirectory(), "tdc" ) == 0 ) )
		{
			static bool bFirstTime = true;
			if ( bFirstTime )
			{
				bFirstTime = false;
				Error( "This game has a minimum requirement of DirectX 9.0 or OpenGL to run properly.\n" );
			}
			return;
		}
	}
