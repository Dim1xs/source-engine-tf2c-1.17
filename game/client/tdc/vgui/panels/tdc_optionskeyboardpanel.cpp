...
#include "controls/tdc_vcontrolslistpanel.h"
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CTDCOptionsKeyboardPanel::CTDCOptionsKeyboardPanel(vgui::Panel *parent, const char *panelName) : CTDCDialogPanelBase(parent, panelName)
{
	bParsed = false;
	...
	m_pSetBindingButton = new CTDCButton( this, "ChangeKeyButton", "" );
	m_pClearBindingButton = new CTDCButton( this, "ClearKeyButton", "" );
}

//-----------------------------------------------------------------------------
// Purpose: Destructor
//-----------------------------------------------------------------------------
CTDCOptionsKeyboardPanel::~CTDCOptionsKeyboardPanel()
{
	DeleteSavedBindings();
}
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::ApplySchemeSettings(IScheme *pScheme)
{
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::OnResetData()
{
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::OnApplyChanges()
{
...
// Purpose: saves out keybinding changes
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::OnSetDefaults()
{
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::CreateKeyBindingList()
{
	// Create the control
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::OnKeyCodeTyped(vgui::KeyCode code)
{
	if (code == KEY_ENTER)
	{
		OnCommand("ChangeKey");
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::OnCommand( const char *command )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::ParseActionDescriptions( void )
{
	char szBinding[256];
...
//-----------------------------------------------------------------------------
KeyValues *CTDCOptionsKeyboardPanel::GetItemForBinding( const char *binding )
{
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::AddBinding( KeyValues *item, const char *keyname )
{
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::ClearBindItems( void )
{
	for (int i = 0; i < m_pKeyBindList->GetItemCount(); i++)
	{
		KeyValues *item = m_pKeyBindList->GetItemData(m_pKeyBindList->GetItemIDFromRow(i));
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::RemoveKeyFromBindItems( KeyValues *org_item, const char *key )
{
	Assert( key && key[ 0 ] );
	if ( !key || !key[ 0 ] )
		return;
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::FillInCurrentBindings( void )
{
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::DeleteSavedBindings( void )
{
	for ( int i = 0; i < BUTTON_CODE_LAST; i++ )
	{
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::SaveCurrentBindings( void )
{
    DeleteSavedBindings();
	for (int i = 0; i < BUTTON_CODE_LAST; i++)
	{
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::BindKey( const char *key, const char *binding )
{
#ifndef _XBOX
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::UnbindKey( const char *key )
{
#ifndef _XBOX
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::ApplyAllBindings( void )
{
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::FillInDefaultBindings( void )
{
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::ItemSelected(int itemID)
{
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::Finish( ButtonCode_t code )
{
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::OnThink()
{
	BaseClass::OnThink();
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::OnKeyCodePressed(vgui::KeyCode code)
{
...
//-----------------------------------------------------------------------------
class CTDCOptionsKeyboardPanelAdvancedDlg : public vgui::Frame
{
	DECLARE_CLASS_SIMPLE( CTDCOptionsKeyboardPanelAdvancedDlg, vgui::Frame );
public:
	CTDCOptionsKeyboardPanelAdvancedDlg( vgui::VPANEL hParent ) : BaseClass( NULL, NULL )
	{
...
//-----------------------------------------------------------------------------
void CTDCOptionsKeyboardPanel::OpenKeyboardAdvancedDialog()
{
	if (!m_OptionsSubKeyboardAdvancedDlg.Get())
	{
		m_OptionsSubKeyboardAdvancedDlg = new CTDCOptionsKeyboardPanelAdvancedDlg(GetVParent());
	}
