...
public:
	...
	void			UpdateStatus( C_TDCWeaponBase *pWeapon );

private:
	ContinuousProgressBar *m_pEffectMeter;
	CExLabel *m_pEffectMeterLabel;

	CHandle<C_TDCWeaponBase> m_hWeapon;
	float m_flOldCharge;
};
...
// Purpose: 
//-----------------------------------------------------------------------------
void CHudItemEffectMeter::UpdateStatus( C_TDCWeaponBase *pWeapon )
{
	...
	if ( m_flOldCharge < 1.0f && flCharge == 1.0f )
	{
		...
		C_BaseEntity::EmitSound( filter, SOUND_FROM_LOCAL_PLAYER, "Player.Recharged" );
	}
...
#define TDC_NUM_EFFECT_METERS 3
...
private:
	CHudItemEffectMeter *m_pItemBars[TDC_NUM_EFFECT_METERS];
	CHudItemEffectMeter *m_pItemCounters[TDC_NUM_EFFECT_METERS];
...
void CHudItemEffects::OnTick( void )
{
	...
	CUtlVector<C_TDCWeaponBase *> aBarWeapons;
	CUtlVector<C_TDCWeaponBase *> aCounterWeapons;

	// Traverse backwards since 1 is the rightmost icon and we want to show slots from left to right.
	for ( int i = TDC_PLAYER_WEAPON_COUNT - 1; i >= 0; i-- )
	{
		C_TDCWeaponBase *pWeapon = pPlayer->GetTDCWeaponBySlot( i );
	...
	if ( m_bUsingLegacyMeter )
	{
		...
		for ( int i = 0; i < TDC_NUM_EFFECT_METERS; i++ )
		{
