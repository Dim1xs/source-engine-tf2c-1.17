...
class CTDCHudDeathNotice : public CHudElement, public vgui::Panel
{
	DECLARE_CLASS_SIMPLE( CTDCHudDeathNotice, vgui::Panel );
public:
	CTDCHudDeathNotice( const char *pElementName );
...
DECLARE_HUDELEMENT( CTDCHudDeathNotice );

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
CTDCHudDeathNotice::CTDCHudDeathNotice( const char *pElementName ) :
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCHudDeathNotice::Init( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCHudDeathNotice::VidInit( void )
{
	m_DeathNotices.RemoveAll();
}

void CTDCHudDeathNotice::ApplySchemeSettings( vgui::IScheme *scheme )
{
...
bool CTDCHudDeathNotice::IsVisible( void )
{
...
//-----------------------------------------------------------------------------
bool CTDCHudDeathNotice::ShouldDraw( void )
{
...
//-----------------------------------------------------------------------------
void CTDCHudDeathNotice::RetireExpiredDeathNotices()
{
...
//-----------------------------------------------------------------------------
void CTDCHudDeathNotice::FireGameEvent( IGameEvent *event )
{
	if ( !g_TDC_PR )
	{
		return;
	...
	if ( FStrEq( pszEventName, "player_death" ) )
	{
		...
		m_DeathNotices[iMsg].Killer.playerColor = ( killer > 0 ) ? g_TDC_PR->GetPlayerColor( killer ) : COLOR_WHITE;
		m_DeathNotices[iMsg].Victim.playerColor = g_TDC_PR->GetPlayerColor( victim );
		...
		if ( assister_name )
		{
			...
			m_DeathNotices[iMsg].Assister.iTeam = ( iAssisterID > 0 ) ? g_TDC_PR->GetTeam( iAssisterID ) : 0;
			m_DeathNotices[iMsg].Assister.playerColor = g_TDC_PR->GetPlayerColor( iAssisterID );
		...
		if ( nDeathFlags & TDC_DEATH_DOMINATION )
		{
			...
			PlayRivalrySounds( iKillerID, iVictimID, TDC_DEATH_DOMINATION );
		}
		if ( ( nDeathFlags & TDC_DEATH_ASSISTER_DOMINATION ) && ( iAssisterID > 0 ) )
		{
			AddAdditionalMsg( iAssisterID, iVictimID, "#Msg_Dominating" );
			PlayRivalrySounds( iAssisterID, iVictimID, TDC_DEATH_DOMINATION );
		}
		if ( nDeathFlags & TDC_DEATH_REVENGE )
		{
			AddAdditionalMsg( iKillerID, iVictimID, "#Msg_Revenge" );
			PlayRivalrySounds( iKillerID, iVictimID, TDC_DEATH_REVENGE );
		}
		if ( ( nDeathFlags & TDC_DEATH_ASSISTER_REVENGE ) && ( iAssisterID > 0 ) )
		{
			AddAdditionalMsg( iAssisterID, iVictimID, "#Msg_Revenge" );
			PlayRivalrySounds( iAssisterID, iVictimID, TDC_DEATH_REVENGE );
		}
		...
		case TDC_DMG_CHAINSAW_BACKSTAB:
	...
	else if ( FStrEq( "teamplay_flag_event", pszEventName ) )
	{
		...
		Assert( iTeam >= FIRST_GAME_TEAM && iTeam < TDC_TEAM_COUNT );
		if ( iTeam >= FIRST_GAME_TEAM && iTeam < TDC_TEAM_COUNT )
		{
...
void CTDCHudDeathNotice::PlayRivalrySounds( int iKillerIndex, int iVictimIndex, int iType )
{
	int iLocalPlayerIndex = GetLocalPlayerIndex();
	...
	else if ( iType == TDC_DEATH_REVENGE )
	{
...
//-----------------------------------------------------------------------------
void CTDCHudDeathNotice::GetLocalizedControlPointName( IGameEvent *event, char *namebuf, int namelen )
{
...
//-----------------------------------------------------------------------------
int CTDCHudDeathNotice::AddDeathNoticeItem()
{
...
//-----------------------------------------------------------------------------
void CTDCHudDeathNotice::DrawText( int x, int y, HFont hFont, Color clr, const wchar_t *szText )
{
...
//-----------------------------------------------------------------------------
void CTDCHudDeathNotice::GetBackgroundPolygonVerts( int x0, int y0, int x1, int y1, int iVerts, vgui::Vertex_t vert[] )
{
...
//-----------------------------------------------------------------------------
CHudTexture *CTDCHudDeathNotice::GetIcon( const char *szIcon, bool bInvert )
{
...
//-----------------------------------------------------------------------------
void CTDCHudDeathNotice::CalcRoundedCorners()
{
...
//-----------------------------------------------------------------------------
void CTDCHudDeathNotice::AddAdditionalMsg( int iKillerID, int iVictimID, const char *pMsgKey )
{
	...
	msg2.Killer.playerColor = g_TDC_PR->GetPlayerColor( iKillerID );
	msg2.Victim.iTeam = g_PR->GetTeam( iVictimID );
	msg2.Victim.playerColor = g_TDC_PR->GetPlayerColor( iVictimID );
...
//-----------------------------------------------------------------------------
Color CTDCHudDeathNotice::GetPlayerColor( const DeathNoticePlayer &playerItem, bool bLocalPlayerInvolved /*= false*/ )
{
