...
IMPLEMENT_CLIENTCLASS_DT( C_TDCPickupItem, DT_TDCPickupItem, CTDCPickupItem )
...
C_TDCPickupItem::C_TDCPickupItem()
{
}
...
// Purpose:
//-----------------------------------------------------------------------------
void C_TDCPickupItem::OnDataChanged( DataUpdateType_t updateType )
{
...
ShadowType_t C_TDCPickupItem::ShadowCastType( void ) 
{
	if ( m_bRespawning )
		 return SHADOWS_NONE;
...
// Purpose:
//-----------------------------------------------------------------------------
int C_TDCPickupItem::DrawModel( int flags )
{
	int ret = 0;
	...
	if ( m_bRespawning && ShouldShowRespawnTimer() )
	{
		...
		Color colFullRespawn = GetRespawnTimerFullColor();
		Color colEmptyRespawn = GetRespawnTimerEmptyColor();
		unsigned char *ubFullColor = (unsigned char *)&colFullRespawn;
		unsigned char *ubEmptyColor = (unsigned char *)&colEmptyRespawn;
		...
		meshBuilder.Color4ubv( ubEmptyColor );
		...
		for ( i = 0; i<8; i++ )
		{
			if ( flProgress < Segments[i].maxProgress )
			{
				...
				// vert 0 is ( 0.5, 0.5 )
				meshBuilder_Full.Color4ubv( ubFullColor );
