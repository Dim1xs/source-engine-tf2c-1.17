...
DECLARE_HUDELEMENT_DEPTH( CTDCStatPanel, 1 );
DECLARE_HUD_MESSAGE( CTDCStatPanel, PlayerStatsUpdate );
...
static CTDCStatPanel *statPanel = NULL;
extern CAchievementMgr g_AchievementMgrTDC;
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
CTDCStatPanel::CTDCStatPanel( const char *pElementName )
	: EditablePanel( NULL, "StatPanel" ), CHudElement( pElementName )
{
	...
	m_iClassCurrentLife = TDC_CLASS_UNDEFINED;
...
//-----------------------------------------------------------------------------
void CTDCStatPanel::LevelShutdown()
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCStatPanel::ApplySettings( KeyValues *inResourceData )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCStatPanel::Init()
{
	// listen for events
	HOOK_HUD_MESSAGE( CTDCStatPanel, PlayerStatsUpdate );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCStatPanel::UpdateStats( int iMsgType )
{
	...
	if ( !g_AchievementMgrTDC.CheckAchievementsEnabled() )
		return;
...
//-----------------------------------------------------------------------------
void CTDCStatPanel::CalcMaxsAndRecords()
{
	if ( m_iClassCurrentLife == TDC_CLASS_UNDEFINED )
		return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCStatPanel::TestStatPanel( TFStatType_t statType, RecordBreakType_t recordType )
{
	C_TDCPlayer *pPlayer = C_TDCPlayer::GetLocalTDCPlayer();
	if ( !pPlayer )
		return;
...
//			we never read it).
//-----------------------------------------------------------------------------
void CTDCStatPanel::WriteStats( void )
{
	if ( !m_bStatsChanged )
		return;
	...
	for( int i = 0; i < m_aClassStats.Count(); i++ )
	{
		...
		if ( ( stat.iPlayerClass >= TDC_CLASS_COUNT_ALL ) || ( stat.iPlayerClass < TDC_FIRST_NORMAL_CLASS ) )
			continue;
	...
	if ( IsX360() )
		V_sprintf_safe( szFilename, "cfg:/tdc_playerstats.dmx" );
	else
		V_sprintf_safe( szFilename, "tdc_playerstats.dmx" );
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTDCStatPanel::ReadStats( void )
{
...
//-----------------------------------------------------------------------------
int CTDCStatPanel::CalcCRC( int iSteamID )
{
	CRC32_t crc;
	CRC32_Init( &crc );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCStatPanel::ShowStatPanel( int iClass, int iTeam, int iCurStatValue, TFStatType_t statType, RecordBreakType_t recordBreakType,
								 bool bAlive )
{
	ClassStats_t &classStats = GetClassStats( iClass );
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCStatPanel::FireGameEvent( IGameEvent * event )
{
	...
	if ( Q_strcmp( "player_spawn", pEventName ) == 0 )
	{
		...
		if ( !C_TDCPlayer::GetLocalTDCPlayer() || ( C_TDCPlayer::GetLocalTDCPlayer()->GetUserID() != iUserID ) )
			return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCStatPanel::ApplySchemeSettings( vgui::IScheme *pScheme )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTDCStatPanel::ShouldDraw( void )
{
...
//-----------------------------------------------------------------------------
void CTDCStatPanel::OnLocalPlayerRemove( C_TDCPlayer *pPlayer )
{
	// this handles the case of map change/server shutdown while player is still alive -- accumulate values for this life.
	if ( pPlayer->IsAlive() && g_AchievementMgrTDC.CheckAchievementsEnabled() )
	{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCStatPanel::ClearStatsInMemory( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCStatPanel::ResetStats( void )
{
	ClearStatsInMemory();
...
//-----------------------------------------------------------------------------
ClassStats_t &CTDCStatPanel::GetClassStats( int iClass )
{
	Assert( statPanel );
	Assert( iClass >= TDC_FIRST_NORMAL_CLASS );
	Assert( iClass < TDC_CLASS_COUNT_ALL );
	int i;
...
//-----------------------------------------------------------------------------
void CTDCStatPanel::UpdateStatSummaryPanel()
{
	if ( guiroot )
	{
...
//-----------------------------------------------------------------------------
void CTDCStatPanel::GetStatValueAsString( int iValue, TFStatType_t statType, char *value, int valuelen )
{
...
//-----------------------------------------------------------------------------
void CTDCStatPanel::MsgFunc_PlayerStatsUpdate( bf_read &msg )
{
