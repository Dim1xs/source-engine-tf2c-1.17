...
bool ShaderEditorHandler::Init()
{
	...
	bool bCreateEditor = ( CommandLine() != NULL ) && (CommandLine()->FindParm("-dev") && CommandLine()->FindParm( "-shadereditor" ) != 0 );
...
#ifdef DEBUG
	// we don't want playtesters OR the end users being able to use my silly prototyping environment (FOR SECUITY PURPOSES!!!) 
	// so we're going to disable ibt on release compile. - Graham
	char modulePath[MAX_PATH * 4];
	Q_snprintf(modulePath, sizeof(modulePath), "%s/bin/client_shadereditor.dll", engine->GetGameDirectory());
	...
	if ( shaderEditorModule )
	{
		...
		else if ( !shaderEdit->Init( factories.appSystemFactory, gpGlobals, sEditMRender, bCreateEditor, bShowPrimDebug, iEnableSkymask ) )
		{
