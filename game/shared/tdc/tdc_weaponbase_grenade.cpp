...
#include "tdc_weaponbase_grenade.h"
...
IMPLEMENT_NETWORKCLASS_ALIASED( TFWeaponBaseGrenade, DT_TDCWeaponBaseGrenade )

BEGIN_NETWORK_TABLE( CTDCWeaponBaseGrenade, DT_TDCWeaponBaseGrenade )
// Client specific.
#ifdef CLIENT_DLL
...
BEGIN_PREDICTION_DATA( CTDCWeaponBaseGrenade )
#ifdef CLIENT_DLL
...
LINK_ENTITY_TO_CLASS( tf_weaponbase_grenade, CTDCWeaponBaseGrenade );
...
CTDCWeaponBaseGrenade::CTDCWeaponBaseGrenade()
{
}
...
// Purpose: Constructor.
//-----------------------------------------------------------------------------
void CTDCWeaponBaseGrenade::Spawn( void )
{
	BaseClass::Spawn();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCWeaponBaseGrenade::Precache()
{
	BaseClass::Precache();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTDCWeaponBaseGrenade::IsPrimed( void )
{
	return m_bPrimed;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTDCWeaponBaseGrenade::Deploy( void )
{
	if ( BaseClass::Deploy() )
	{
		Prime();
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCWeaponBaseGrenade::Prime() 
{
	m_flThrowTime = gpGlobals->curtime + GetTDCWpnData().m_flPrimerTime;
	m_bPrimed = true;

#ifndef CLIENT_DLL
	if ( GetWeaponID() != TDC_WEAPON_GRENADE_SMOKE_BOMB )
	{
		// Get the player owning the weapon.
		CTDCPlayer *pPlayer = ToTFPlayer( GetPlayerOwner() );
		if ( !pPlayer )
			return;
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCWeaponBaseGrenade::Throw() 
{
	if ( !m_bPrimed )
		return;
	...
	CTDCPlayer *pPlayer = ToTFPlayer( GetOwner() );
	if ( !pPlayer )
		return;
	...
	bool bExplodingOnDeath = ( !pPlayer->IsAlive() || pPlayer->StateGet() == TDC_STATE_DYING );
	...
	else
	{
		...
		CTDCWeaponBaseGrenadeProj *pGrenade = EmitGrenade( vecSrc, vecAngles, vecThrow, AngularImpulse( 600, random->RandomInt( -1200, 1200 ), 0 ), pPlayer, 0.0 );
		if ( pGrenade )
		{
			pGrenade->Detonate();
		}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTDCWeaponBaseGrenade::ShouldDetonate( void )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCWeaponBaseGrenade::ItemPostFrame()
{
	CTDCPlayer *pPlayer = ToTFPlayer( GetPlayerOwner() );
	if ( !pPlayer )
		return;
...
bool CTDCWeaponBaseGrenade::ShouldLowerMainWeapon( void )
{
	return GetTDCWpnData().m_bLowerWeapon;
}
...
// Purpose: 
//-----------------------------------------------------------------------------
bool CTDCWeaponBaseGrenade::ShouldDraw( void )
{
...
BEGIN_DATADESC( CTDCWeaponBaseGrenade )
END_DATADESC()

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
void CTDCWeaponBaseGrenade::HandleAnimEvent( animevent_t *pEvent )
{
...
// Purpose: 
//-----------------------------------------------------------------------------
CTDCWeaponBaseGrenadeProj *CTDCWeaponBaseGrenade::EmitGrenade( Vector vecSrc, QAngle vecAngles, Vector vecVel, AngularImpulse angImpulse, CBasePlayer *pPlayer, float flTime, int iFlags )
{
