...
//
//=============================================================================
#ifndef TDC_GAMESTATS_SHARED_H
#define TDC_GAMESTATS_SHARED_H
#ifdef _WIN32
#pragma once
#endif
#include "cbase.h"
...
{
	TDC_GAMESTATS_FILE_VERSION = 006,
	TDC_GAMESTATS_MAGIC = 0xDEADBEEF
};
...
struct TDC_Gamestats_Version_t
{
	int m_iMagic;			// always TDC_GAMESTATS_MAGIC
	int m_iVersion;
};

struct TDC_Gamestats_ClassStats_t
{
	...
	void Accumulate( TDC_Gamestats_ClassStats_t &other )
	{
...
struct TDC_Gamestats_WeaponStats_t
{
	...
	void Accumulate( TDC_Gamestats_WeaponStats_t &other )
	{
...
//
struct TDC_Gamestats_LevelStats_t
{
public:

	TDC_Gamestats_LevelStats_t();
	~TDC_Gamestats_LevelStats_t();
	TDC_Gamestats_LevelStats_t( const TDC_Gamestats_LevelStats_t &stats );

	// Level start and end
	void Init( const char *pszMapName, int nIPAddr, short nPort, float flStartTime );
	void Shutdown( float flEndTime );

	void Accumulate( TDC_Gamestats_LevelStats_t *pOther )
	{
...
public:
	...
	TDC_Gamestats_ClassStats_t		m_aClassStats[TDC_CLASS_COUNT_ALL];	// Vector of class data
	TDC_Gamestats_WeaponStats_t		m_aWeaponStats[WEAPON_COUNT];	// Vector of weapon data
	// Temporary data.
	bool							m_bInitialized;		// Has the map Map Stat Data been initialized.
	float							m_flRoundStartTime;
	int								m_iPeakPlayerCount[TDC_TEAM_COUNT];
};
...
struct TFReportedStats_t
{
	...
	~TFReportedStats_t();
	void Clear();
	TDC_Gamestats_LevelStats_t	*FindOrAddMapStats( const char *szMapName );
#ifdef GAME_DLL
	void AppendCustomDataToSaveBuffer( CUtlBuffer &SaveBuffer );
	bool LoadCustomDataFromBuffer( CUtlBuffer &LoadBuffer );
#endif 

	TDC_Gamestats_LevelStats_t								*m_pCurrentGame;
	CUtlDict<TDC_Gamestats_LevelStats_t, unsigned short>		m_dictMapStats;
};
