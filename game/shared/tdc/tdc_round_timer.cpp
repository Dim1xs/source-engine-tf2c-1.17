...
#ifdef GAME_DLL
ConVar tdc_overtime_nag( "tdc_overtime_nag", "0", FCVAR_NOTIFY, "Announcer overtime nag." );
#endif
...
LINK_ENTITY_TO_CLASS( game_round_timer, CTeamRoundTimer );
PRECACHE_REGISTER( game_round_timer );
...
{
	return ( TDCGameRules()->State_Get() == GR_STATE_PREROUND );
}
...
int CTeamRoundTimer::GetTimeWarningMessage( int nWarning )
{
	...
	case RT_WARNING_60SECS:
		...
		{
			iMessage = TDC_ANNOUNCER_ROUNDBEGINS_60SEC;
		}
		else
		{
			iMessage = TDC_ANNOUNCER_ROUNDENDS_60SEC;
		}
		break;
	case RT_WARNING_30SECS:
		if ( nState == RT_STATE_SETUP )
		{
			iMessage = TDC_ANNOUNCER_ROUNDBEGINS_30SEC;
		}
		else
		{
			iMessage = TDC_ANNOUNCER_ROUNDENDS_30SEC;
		}
		break;
	case RT_WARNING_10SECS:
		if ( nState == RT_STATE_SETUP )
		{
			iMessage = TDC_ANNOUNCER_ROUNDBEGINS_10SEC;
		}
		else
		{
			iMessage = TDC_ANNOUNCER_ROUNDENDS_10SEC;
		}
		break;
	case RT_WARNING_5SECS:
		if ( nState == RT_STATE_SETUP )
		{
			iMessage = TDC_ANNOUNCER_ROUNDBEGINS_5SEC;
		}
		else
		{
			iMessage = TDC_ANNOUNCER_ROUNDENDS_5SEC;
		}
		break;
	case RT_WARNING_4SECS:
		if ( nState == RT_STATE_SETUP )
		{
			iMessage = TDC_ANNOUNCER_ROUNDBEGINS_4SEC;
		}
		else
		{
			iMessage = TDC_ANNOUNCER_ROUNDENDS_4SEC;
		}
		break;
	case RT_WARNING_3SECS:
		if ( nState == RT_STATE_SETUP )
		{
			iMessage = TDC_ANNOUNCER_ROUNDBEGINS_3SEC;
		}
		else
		{
			iMessage = TDC_ANNOUNCER_ROUNDENDS_3SEC;
		}
		break;
	case RT_WARNING_2SECS:
		if ( nState == RT_STATE_SETUP )
		{
			iMessage = TDC_ANNOUNCER_ROUNDBEGINS_2SEC;
		}
		else
		{
			iMessage = TDC_ANNOUNCER_ROUNDENDS_2SEC;
		}
		break;
	case RT_WARNING_1SECS:
		if ( nState == RT_STATE_SETUP )
		{
			iMessage = TDC_ANNOUNCER_ROUNDBEGINS_1SEC;
		}
		else
		{
			iMessage = TDC_ANNOUNCER_ROUNDENDS_1SEC;
		}
		break;
	default:
		iMessage = -1;
...
void CTeamRoundTimer::SendTimeWarning( int nWarning )
{
	...
	if ( !m_bTimerPaused && m_bAutoCountdown && !TDCGameRules()->IsInWaitingForPlayers() )
	{
		bool bShouldPlaySound = false;

		if ( TDCGameRules()->IsInTournamentMode() == true && TDCGameRules()->IsInStopWatch() == true )
		{
...
void CTeamRoundTimer::RoundTimerSetupThink( void )
{
	...
	if ( TDCGameRules()->IsInPreMatch() == true && IsDisabled() == false )
	{
		inputdata_t data;
		InputDisable( data );
	...
	float flTime = GetTimeRemaining();
	TDCGameRules()->SetOvertime( false );
	...
	if ( flTime <= 0.0f && m_bFireFinished )
	{
		...
		if ( ObjectiveResource()->IsActiveTimer( this ) && !TDCGameRules()->IsInWaitingForPlayers() )
		{
...
void CTeamRoundTimer::RoundTimerThink( void )
{
	...
	if ( TDCGameRules()->RoundHasBeenWon() ||
		TDCGameRules()->IsLoadingBugBaitReport() )
	{
	...
	if ( flTime <= 0.0f && m_bFireFinished )
	{
		...
		if ( !TeamplayGameRules()->TimerMayExpire() )
		{
			...
			if ( ShowInHud() )
			{
				if ( !TDCGameRules()->InOvertime() )
				{
					TDCGameRules()->SetOvertime( true );
				}
				else
				{
					if ( tdc_overtime_nag.GetBool() && ( gpGlobals->curtime > m_flNextOvertimeNag ) )
					{
...
void CTeamRoundTimer::InputRoundSpawn( inputdata_t &input )
{
	...
	if ( !m_bStartPaused && !TDCGameRules()->IsInWaitingForPlayers() )
	{
		ResumeTimer();
	}
}
...
void CTeamRoundTimer::AddTimerSeconds( int iSecondsToAdd, int iTeamResponsible /* = TEAM_UNASSIGNED*/ )
{
	...
	if ( TDCGameRules()->State_Get() != GR_STATE_RND_RUNNING && TDCGameRules()->State_Get() != GR_STATE_TEAM_WIN )
		return;
	...
	if ( ( ObjectiveResource() && ObjectiveResource()->IsActiveTimer( this ) ) )
	{
		if ( !TDCGameRules()->InStalemate() && !TDCGameRules()->RoundHasBeenWon() )
		{
			if ( iTeamResponsible >= LAST_SHARED_TEAM+1 )
			{
				for ( int iTeam = FIRST_GAME_TEAM; iTeam < GetNumberOfTeams(); iTeam++ )
				{
					...
					{
						g_TFAnnouncer.Speak( iTeam, TDC_ANNOUNCER_TIMEADDED_TEAM );
					}
					else
					{
						g_TFAnnouncer.Speak( iTeam, TDC_ANNOUNCER_TIMEADDED_ENEMY );
					}
				}
			}
			else
			{
				g_TFAnnouncer.Speak( TDC_ANNOUNCER_TIMEADDED );
			}
		}
...
{ 
	...
	if ( ObjectiveResource()->IsActiveTimer( this ) && !nShow )
	{
		SetActiveTimer( NULL );
	}
	else if ( nShow == 1 )
