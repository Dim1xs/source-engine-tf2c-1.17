...
// Input  :  - 
//-----------------------------------------------------------------------------
TDC_Gamestats_LevelStats_t::TDC_Gamestats_LevelStats_t()
{
...
// Input  :  - 
//-----------------------------------------------------------------------------
TDC_Gamestats_LevelStats_t::TDC_Gamestats_LevelStats_t( const TDC_Gamestats_LevelStats_t &stats )
{
...
//			flStartTime - 
//-----------------------------------------------------------------------------
void TDC_Gamestats_LevelStats_t::Init( const char *pszMapName, int nIPAddr, short nPort, float flStartTime  )
{
	// Initialize.
...
// Input  : flEndTime - 
//-----------------------------------------------------------------------------
void TDC_Gamestats_LevelStats_t::Shutdown( float flEndTime )
{
}
...
// Purpose: 
// Input  : *szMapName - 
// Output : TDC_Gamestats_LevelStats_t
//-----------------------------------------------------------------------------
TDC_Gamestats_LevelStats_t *TFReportedStats_t::FindOrAddMapStats( const char *szMapName )
{
...
void TFReportedStats_t::AppendCustomDataToSaveBuffer( CUtlBuffer &SaveBuffer )
{
	...
	TDC_Gamestats_Version_t versionLump;
	versionLump.m_iMagic = TDC_GAMESTATS_MAGIC;
	versionLump.m_iVersion = TDC_GAMESTATS_FILE_VERSION;
	...
	{
		// Get the current map.
		TDC_Gamestats_LevelStats_t *pCurrentMap = &m_dictMapStats[iMap];
		Assert( pCurrentMap );

		// Write out the lumps.
		CBaseGameStats::AppendLump( MAX_LUMP_COUNT, SaveBuffer, TFSTATS_LUMP_MAPHEADER, 1, sizeof( TDC_Gamestats_LevelStats_t::LevelHeader_t ), static_cast<void*>( &pCurrentMap->m_Header ) );
		CBaseGameStats::AppendLump( MAX_LUMP_COUNT, SaveBuffer, TFSTATS_LUMP_MAPDEATH, pCurrentMap->m_aPlayerDeaths.Count(), sizeof( TDC_Gamestats_LevelStats_t::PlayerDeathsLump_t ), static_cast<void*>( pCurrentMap->m_aPlayerDeaths.Base() ) );
		CBaseGameStats::AppendLump( MAX_LUMP_COUNT, SaveBuffer, TFSTATS_LUMP_MAPDAMAGE, pCurrentMap->m_aPlayerDamage.Count(), sizeof( TDC_Gamestats_LevelStats_t::PlayerDamageLump_t ), static_cast<void*>( pCurrentMap->m_aPlayerDamage.Base() ) );
...
bool TFReportedStats_t::LoadCustomDataFromBuffer( CUtlBuffer &LoadBuffer )
{
	...
	if ( versionLump.m_iMagic != TDC_GAMESTATS_MAGIC )
	{
		Msg( "Incorrect magic # in version header.  Expected %x, got %x.  Skipping file.\n", TDC_GAMESTATS_MAGIC, versionLump.m_iMagic );
		return false;
	}
	if ( versionLump.m_iVersion != TDC_GAMESTATS_FILE_VERSION )
	{
		Msg( "Mismatched file version.  Expected file version %d, got %d. Skipping file.\n", TDC_GAMESTATS_FILE_VERSION, versionLump.m_iVersion  );
		return false;
	}
	...
	while( CBaseGameStats::GetLumpHeader( MAX_LUMP_COUNT, LoadBuffer, iLump, iLumpCount ) )
	{
		...
		case TFSTATS_LUMP_MAPHEADER: 
			{
				TDC_Gamestats_LevelStats_t::LevelHeader_t header;
				CBaseGameStats::LoadLump( LoadBuffer, iLumpCount, sizeof( TDC_Gamestats_LevelStats_t::LevelHeader_t ), &header );
		...
		case TFSTATS_LUMP_MAPDEATH:
			{
				CUtlVector<TDC_Gamestats_LevelStats_t::PlayerDeathsLump_t> playerDeaths;

				playerDeaths.SetCount( iLumpCount );
				CBaseGameStats::LoadLump( LoadBuffer, iLumpCount, sizeof( TDC_Gamestats_LevelStats_t::PlayerDeathsLump_t ), static_cast<void*>( playerDeaths.Base() ) );
				if ( m_pCurrentGame )
				{
		...
		case TFSTATS_LUMP_MAPDAMAGE:
			{
				CUtlVector<TDC_Gamestats_LevelStats_t::PlayerDamageLump_t> playerDamage;

				playerDamage.SetCount( iLumpCount );
				CBaseGameStats::LoadLump( LoadBuffer, iLumpCount, sizeof( TDC_Gamestats_LevelStats_t::PlayerDamageLump_t ), static_cast<void*>( playerDamage.Base() ) );
				if ( m_pCurrentGame )
				{
		...
		case TFSTATS_LUMP_CLASS:
			{
				...
				if ( iLumpCount == ARRAYSIZE( m_pCurrentGame->m_aClassStats ) )
				{
					...
					{
						TDC_Gamestats_ClassStats_t &classStats = m_pCurrentGame->m_aClassStats[i];
...
#if 0
					// quick sanity check on some data -- we get some stat files that start out OK but are corrupted later in the file
					if ( ( m_pCurrentGame->m_aWeaponStats[TDC_WEAPON_MEDIGUN].iShotsFired < 0 ) || ( m_pCurrentGame->m_aWeaponStats[TDC_WEAPON_MEDIGUN].iShotsFired > 100000 )
						|| ( m_pCurrentGame->m_aWeaponStats[TDC_WEAPON_FLAMETHROWER_ROCKET].iShotsFired != 0 ) ) // check that unused weapon has 0 shots
					{
						return false;
					}
#endif
